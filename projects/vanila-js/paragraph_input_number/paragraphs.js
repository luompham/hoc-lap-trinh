...
const form = document.querySelector('form.lorem-form');
const input = document.querySelector('input#amount');
const button = document.querySelector('button');
const content = document.querySelector('div.lorem-content');
...
form.addEventListener('submit', (e) => {
    ...
    if (isNaN(value) || value <= 0 || value > 9) {
        ...
        content.innerHTML = `<p class='text'>${texts[randomIndex]}</p>`
    } else {

        // console.log(texts.slice(0, value))
        const newTexts = texts.slice(0, value)

        const textHTML = newTexts.map(text => {
